...
* \file
*
* \copydoc Opm::MethanogenicBacteriaProblem
*/
#ifndef EWOMS_METHOGENIC_BACTERIA_PROBLEM_HH
#define EWOMS_METHOGENIC_BACTERIA_PROBLEM_HH

#include <opm/models/io/cubegridvanguard.hh>

#include <opm/material/fluidsystems/MGBFluidSystem.hpp>
...
class MethanogenicBacteriaProblem;
}  // namespace Opm

namespace Opm::Properties {

namespace TTag {
struct  MethanogenicBacteriaBaseProblem {};
}  // namespace TTag
...
// Set the grid type
template<class TypeTag>
struct Grid<TypeTag, TTag::MethanogenicBacteriaBaseProblem> { using type = Dune::YaspGrid<1>; };

// Grid vanguard
template<class TypeTag>
struct Vanguard<TypeTag, TTag::MethanogenicBacteriaBaseProblem> { using type = Opm::CubeGridVanguard<TypeTag>; };

// Set the problem property
template<class TypeTag>
struct Problem<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{ using type = Opm::MethanogenicBacteriaProblem<TypeTag>; };

// Set fluid system
template<class TypeTag>
struct FluidSystem<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
private:
    using Scalar = GetPropType<TypeTag, Properties::Scalar>;
public:
    using type = Opm::MGBFluidSystem<Scalar>;
};

// Set fluid-matrix interactions
template<class TypeTag>
struct MaterialLaw<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
private:
    using Scalar = GetPropType<TypeTag, Properties::Scalar>;
...
// Disable gravity
template<class TypeTag>
struct EnableGravity<TypeTag, TTag::MethanogenicBacteriaBaseProblem> { static constexpr bool value = false; };

// Default grid
template<class TypeTag>
struct DomainSizeX<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
    using type = GetPropType<TypeTag, Scalar>;
    static constexpr type value = 1.0;
};
template<class TypeTag>
struct DomainSizeY<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
    using type = GetPropType<TypeTag, Scalar>;
    static constexpr type value = 1.0;
};
template<class TypeTag>
struct DomainSizeZ<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
    using type = GetPropType<TypeTag, Scalar>;
    static constexpr type value = 1.0;
};
...
template<class TypeTag>
struct CellsX<TypeTag, TTag::MethanogenicBacteriaBaseProblem> { static constexpr int value = 250; };
template<class TypeTag>
struct CellsY<TypeTag, TTag::MethanogenicBacteriaBaseProblem> { static constexpr int value = 1; };
template<class TypeTag>
struct CellsZ<TypeTag, TTag::MethanogenicBacteriaBaseProblem> { static constexpr int value = 1; };

// Default end time of the simulation
template<class TypeTag>
struct EndTime<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
    using type = GetPropType<TypeTag, Scalar>;
...
template<class TypeTag>
struct InitialTimeStepSize<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
    using type = GetPropType<TypeTag, Scalar>;
...
template<class TypeTag>
struct Temperature<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
    using type = GetPropType<TypeTag, Scalar>;
...
template<class TypeTag>
struct InitPressure<TypeTag, TTag::MethanogenicBacteriaBaseProblem>
{
    using type = GetPropType<TypeTag, Scalar>;
...
template <class TypeTag>
struct Injrate<TypeTag, TTag::MethanogenicBacteriaBaseProblem> {
    using type = GetPropType<TypeTag, Scalar>;
...
template<class TypeTag>
struct SimulationName<TypeTag, TTag::MethanogenicBacteriaBaseProblem> { static constexpr auto value = "mgbacteria"; };
...
* \brief Problem where hydrogen (H2) gas is injected in a aqueous solution with methanogenic bacteria
*
...
*/
template <class TypeTag>
class MethanogenicBacteriaProblem : public GetPropType<TypeTag, Properties::BaseProblem>
{
    ...
    enum { CH4Idx = FluidSystem::CH4Idx };
...
public:
    ...
    */
    MethanogenicBacteriaProblem(Simulator& simulator)
    ...
    */
                ...
                unsigned timeIdx) const
    {
        ...
        Evaluation xCO2 = intQuants.fluidState().moleFraction(liquidPhaseIdx, CO2Idx);
        Scalar kg_max = 1.643e-5;
        Scalar kH2 = 1.094e-7;
        Scalar kCO2 = 3.188e-6;

        Evaluation kg = kg_max * (xH2 / (kH2 + xH2)) * (xCO2/ (xCO2 + kCO2));

        // Microbial decay (constant)
        Evaluation kd = 8.565e-7;

        // Insert rates for water, H2, CO2, CH4, and bacteria
        ...
        Scalar yield = 1.376e12;
        Evaluation rg = poro * Sw * rhoMolar * xBact * kg / yield;
        rate[conti0EqIdx + H2OIdx] = 2 * rg;
        rate[conti0EqIdx + H2Idx] = -4 * rg;
        rate[conti0EqIdx + CO2Idx] = -1 * rg;
        rate[conti0EqIdx + CH4Idx] = 1 * rg;
        rate[conti0EqIdx + BactIdx] = (Sw * kg - kd) * rhoMolar * xBact;
    }
...
private:
                            ...
                            unsigned timeIdx) const
    {
        ...
        Scalar xCH4 = 0.05;
        Scalar xBact = 0.01;
        Scalar xH2O = 1 - xH2 - xCO2 - xCH4 - xBact;
        fs.setMoleFraction(liquidPhaseIdx, H2Idx, xH2);
        fs.setMoleFraction(liquidPhaseIdx, CO2Idx, xCO2);
        fs.setMoleFraction(liquidPhaseIdx, CH4Idx, xCH4);
        fs.setMoleFraction(liquidPhaseIdx, BactIdx, xBact);
        fs.setMoleFraction(liquidPhaseIdx, H2OIdx, xH2O);
